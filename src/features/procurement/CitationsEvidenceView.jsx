...
export default function CitationsEvidenceView({
  ...
  const [activeTab, setActiveTab] = useState('all'); // 'all', 'citations', 'evidence'
  const [evidenceFilter, setEvidenceFilter] = useState('ALL');
  const [copiedCitationId, setCopiedCitationId] = useState(null);

  // Combine recommendations or fall back to matchedStandards if recommendations are empty
  ...
  // If backend returned evidence items, use them; if empty, synthesize evidence from standardList
            ...
            <div className="grid grid-cols-1 gap-4">
              {standardList.map((rec) => {
                ...
                return (
                  ...
                  >
                    {/* Top Row: Code, Title, Relevance Badge */}
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-slate-200/80 text-xs">
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 text-xs font-semibold text-brand-blue hover:text-blue-800 transition-colors"
                      >
